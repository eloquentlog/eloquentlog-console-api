...
impl<'v> FromData<'v> for UserLogin {
    ...
    fn transform(_: &Request, data: Data) -> Transform<Outcome<Self::Owned, Self::Error>> {
        ...
        let mut string = String::with_capacity((USER_LOGIN_LENGTH_LIMIT / 2) as usize);
        let outcome = match  stream.read_to_string(&mut string) {
            Ok(_) => Success(string),
            Err(e) => Failure((Status::InternalServerError, UserLoginError::Io(e))),
        };

        Transform::Borrowed(outcome)
    }

    fn from_data(_: &Request, outcome: Transformed<'v, Self>) -> Outcome<Self, Self::Error> {
        let input  = outcome.borrowed()?;
        let user_login: UserLogin  = match serde_json::from_str(input) {
            Ok(v) => v,
            Err(_) => {
                return Failure((Status::InternalServerError, UserLoginError::Empty));
            },
        };
...
pub fn login(_conn: DbConn, user_login: Json<UserLogin>) ->
    Result<Response, Response> {

    let user = User{username: "".to_string(), password_hash: vec!{}};
